	...
	draw : function draw() {
		...
		do {
			...
			do {
				...
				if(this.settings.excludeWeeks==="-1"){//门票
					//klass.push(classes.dayInMonth);
					//if (!(this.blackout(day) || this.direction(day) || (day.month() != month.month() && opts.dayOutOfMonthClickable === false)) || s>0) klass.push(classes.dayActive);
					if(this.settings.selectEndDate==="-1" && this.settings.selectStartDate==="-1"){
						if (!(this.blackout(day) || this.direction(day) || (day.month() != month.month() && opts.dayOutOfMonthClickable === false)) || s>0) klass.push(classes.dayActive);
					}else if(this.settings.selectEndDate!=="-1" && day.toDate().getTime()<new Date(this.settings.selectEndDate).getTime() && this.settings.selectStartDate!=="-1" && day.toDate().getTime()>=new Date(this.settings.selectStartDate).getTime()){
						if (!(this.blackout(day) || this.direction(day) || (day.month() != month.month() && opts.dayOutOfMonthClickable === false)) || s>0) klass.push(classes.dayActive);
					}
				}else{
					var _date_ = day.toDate();    //此处也可以写成 17/07/2014 一样识别    也可以写成 07-17-2014  但需要正则转换   
					var _day_ = new Date(Date.parse(_date_));   //需要正则转换的则 此处为 ： var day = new Date(Date.parse(date.replace(/-/g, '/')));  
					var _today_ = new Array('0','1','2','3','4','5','6');  
					var _week_ = _today_[_day_.getDay()]; 
					
					var weeks = this.settings.excludeWeeks.split(',');
					var bo = true;

...
<body>
   <div>
     <div class="header">
		<div class="header_left"> <a href="javascript:window.history.go(-1);" class="header_icon_back"></a></div>
		<h1 class="header_title">${columnNews.name}</h1>
		<div class="header_right"><a href="/oauth/website.do?comid=${columnNews.companyId}" class="header_icon_bar"></a></div>
      </div>

		<div class="graphiclistbox">
			<ul class="graphiclist_f" id="horizlist">
...
$(function(){
	var $loading = $("#loading");
	var $dataContainer = $(".graphiclist_f");
		...
		"htmlTemp": function (data){ //func 数据的html结构 接受了返回的data
			...
			if (l > 0) {
				...
				htmls.push('<li>'
						  ...
						  +'		<h3 class="title">'+r[i].title+'</h3><span class="time">'+zdate+'</span>'
						  +'	</a>'
						  +'</li>');
